...
Por que el preprocesador escribio el contenido de los archivos del "#include" en el codigo fuente
...
El comando grep busca lineas que coincidan con el patron "Archivo fuente principal", el problema es que en el archivo programa.i no se encuentra, por lo que no devuelve nada
...
Son marcadores de linea que el preprocesador inserta para registrar el origen de cada uno de los bloques. El bloque que contiene la declaracion de printf esta en el archivo "stdio.h"
...
En terminos generales esta iniciando un proceso y estableciendo el espacio de memoria a utilizar por el mismo
...
> **R:**
En programa.o se la muestra con U ya que no esta definida en el archivo, es una funcion referenciada del archivo matematica.o, en este ultimo aparece con una T ya que es un codigo ejecutable definido en el archivo
...
> **R:**
Los simbolos que quedan en U incluso en el ejecutable final son funciones de la biblioteca dinamica del sistema libc.dylib. El enlazador gcc no las copia ya que luego el cargador dinamico  (dyld/ld.so) las resuelve cuando el programa se ejecuta
...
> **R:**
Una macro funcion es una directiva para el preprocesador, simplemente expande el texto de "CUADRADO(x)" a "x*x", esta no verifica tipos y desaparece durante la etapa de preprocesamiento
Una funcion real tiene su propia existencia en la memoria, es un bloque de instrucciones que necesita verificar los tipos para funcionar. Estas no desaparecen.
---
...
> **R:**
El simbolo T indica que la funcion o variable se encuentra definida en la seccion de text, por otro lado la D indica que esta definida en la seccion de data (para variables globales)
---
...
> **R:**
linux-vdso.so.1 (0x00007ffcbf767000)
	libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6 (0x00007f85cac00000)
	/lib64/ld-linux-x86-64.so.2 (0x00007f85cae3b000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:**
libc es una bliblioteca dinamica del sistema, por lo tanto el enlazador no las copia en el archivo ya que de eso se encarga el cargador dinamico durante la ejecucion del programa
---
